...
# OVS
INSTALL_LOGGER = '{0}.install'.format(OVS_LOGGER)
UNINSTALL_LOGGER = '{0}.uninstall'.format(OVS_LOGGER)
...
# Following loggers are best configured to always log to a file
LOGGER_FILE_MAP_ALWAYS_FILE = {UPDATE_LOGGER: os.path.join(LOG_PATH, 'update.log'),
                               INSTALL_LOGGER: os.path.join(LOG_PATH, 'install.log'),
                               UNINSTALL_LOGGER: os.path.join(LOG_PATH, 'uninstall.log')}

LOGGER_FILE_MAP.update(LOGGER_FILE_MAP_ALWAYS_FILE)
