        ...
        try:
            ...
            if first_node is False:
                for cluster in SetupController.arakoon_clusters:
                    ...
                    config.load_config(SSHClient.load(master_ip, known_passwords[master_ip]))
            ...
            if first_node:
                SetupController._setup_first_node(cluster_ip=cluster_ip,
                                                  unique_id=unique_id,
...
""".format(mapping[ip], ip)
    ...
    @staticmethod
    def _setup_first_node(cluster_ip, unique_id, mountpoints, cluster_name, node_name, hypervisor_info, arakoon_mountpoint, enable_heartbeats, writecaches):
        """
...
EOF
    ...
    @staticmethod
    def _promote_node(cluster_ip, master_ip, cluster_name, nodes, unique_id, mountpoints, arakoon_mountpoint, writecaches):
        """
        ...
        for cluster in SetupController.arakoon_clusters:
            config = ArakoonClusterConfig(cluster)
            config.load_config(SSHClient.load(master_ip))
...
"""
    ...
    def apply_flexible_disk_layout(client, auto_config=False, default=None):
        ...
        def check_percentages(percentage_mapping, device_to_check):
            total_percentage_assigned = sum([percent for percent in percentage_mapping[device_to_check].itervalues()])
