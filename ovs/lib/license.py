    ...
    def validate(license_string):
        ...
        try:
            ...
            for component_data in data:
                ...
                valid_until = float(component_data['valid_until']) if 'valid_until' in component_data else None
                signature = component_data['signature'] if 'signature' in component_data else None
                ...
                if len(validate_functions) == 1 and len(apply_functions) == 1:
                    try:
                        valid, metadata = validate_functions[0](component=component, data=data, valid_until=valid_until, signature=signature)
                    except Exception, ex:
        ...
        try:
            ...
            for component_data in data:
                ...
                if len(validate_functions) == 1 and len(apply_functions) == 1:
                    valid, metadata = validate_functions[0](component=component, data=data, valid_until=valid_until, signature=signature)
                    if valid is True:
                        success = apply_functions[0](component=component, data=data, valid_until=valid_until, signature=signature)
                        if success is True:
