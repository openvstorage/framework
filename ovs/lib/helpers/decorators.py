...
def ovs_task(**kwargs):
    ...
    def wrapper(f):
        ...
        if ensure_single_info != {}:
            ensure_single_container = EnsureSingleContainer(task_name=kwargs['name'], **ensure_single_info)
...
class EnsureSingleContainer(object):
    ...
    """
    def __init__(self, task_name, mode, extra_task_names=None, global_timeout=300, callback=None, ignore_arguments=None):
        """
        ...
        self.global_time = global_timeout
...
class EnsureSingle(object):
    ...
    def lock_default_mode(self):
        ...
        :return: True if the lock was acquirable
        :rtype: bool
            ...
            for task in self.ensure_single_container.task_names:
                ...
                if self.persistent_client.exists(key_to_check):
                    ...
                    if self.async_task or not self.ensure_single_container.callback:
                        discard_message = 'Execution of task {0} discarded'.format(self.ensure_single_container.task_name)
        ...
        try:
            ...
            success = False
            raise
            # @todo the error is ignored here??
        finally:
            if success:
    ...
    def do_callback(self, *args, **kwargs):
        ...
        return self.ensure_single_container.callback(*args, **kwargs)
...
# def _ensure_single_deduped(ensure_single_container):
#     """
#     Decorator ensuring a new task cannot be started in case a certain task is
#     running, scheduled or reserved.
#
...
#     Mode: DEFAULT: De-duplication based on the task's name. If any new task with the same name is scheduled it will be discarded
#
#     :param ensure_single_container: Ensure single instance
#     :type ensure_single_container: EnsureSingle
#     :return: Pointer to function
#     :rtype: func
#     """
#
...
#
#         @wraps(f)
#         def ensure_single_deduped(self, *args, **kwargs):
#             """
#             Wrapped function
#             :param self: With bind=True, the celery task result itself is passed in
#             :param args: Arguments without default values
#             :param kwargs: Arguments with default values
#             """
#             ensure_single_container.set_task(self)
#             ensure_single_container.validate_no_extra_names()
#
#             # Update kwargs with args
#             sleep = 1 if unittest_mode is False else 0.1
#             timeout = kwargs.pop('ensure_single_timeout', 10 if unittest_mode is True else global_timeout)
...
#                         if async_task is True:  # Not waiting for other jobs to finish since asynchronously
#                             log_message(
#                                 'Execution of task {0} {1} discarded because of identical parameters'.format(task_name,
#                                                                                                              params_info))
#                             ensure_single_container.unittest_set_state_discarded()
#                             return None
#
#                         # If executed inline (sync), execute callback if any provided
#                         if callback is not None:
#                             log_message(
#                                 'Execution of task {0} {1} in progress, executing callback function'.format(task_name,
#                                                                                                             params_info))
#                             ensure_single_container.unittest_set_state_callback()
#                             return callback(*args, **kwargs)
#
#                         # Let's wait for 2nd job in queue to have finished if no callback provided
#                         slept = 0
...
#                                                                                                              slept + sleep))
#                             values = list(persistent_client.get_multi([persistent_key], must_exist=False))
#                             if values[0] is None or item['timestamp'] not in [value['timestamp'] for value in values[0]['values']]:
...
#                                 raise EnsureSingleTimeoutReached('Ensure single {0} mode - ID {1} - Task {2} could not be started within timeout of {3}s'.format(
#                                     mode,
#                                     now,
#                                     task_name,
#                                     timeout))
...
#                             if slept != 0:
#                                 log_message(
#                                     'Task {0} {1} had to wait {2} seconds before being able to start'.format(task_name,
#                                                                                                              params_info,
#                                                                                                              slept))
#                             if unittest_mode is True:
#                                 Decorators.unittest_thread_info_by_name[thread_name] = ('EXECUTING', None)
...
#                                                   'timestamp': now})
#                     log_message(
#                         'Could not start task {0} {1}, within expected time ({2}s). Removed it from queue'.format(
#                             task_name, params_info, timeout),
#                         level='error')
#                     if unittest_mode is True:
#                         Decorators.unittest_thread_info_by_name[thread_name] = (
#                         'EXCEPTION', 'Could not start within timeout of {0}s while queued'.format(timeout))
