...
class Configure():
    ...
    def init_storagerouter(vmachineguid, vpool_name):
        ...
        if volumedriver_backend_type == 'LOCAL':
            ...
            fstab = Fstab()
            fstab.remove_config_by_directory(distributed_filesystem_mountpoint)
            fstab.add_config('id=admin,conf=/etc/ceph/ceph.conf', distributed_filesystem_mountpoint, 'fuse.ceph', 'defaults,noatime', '0', '2')
