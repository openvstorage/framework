...
class Client(UnicodeMixin):
    ...
    def add_extern_service(self, name, portname, binding, location):
        ...
        @type location: str
        """
        service = self.wsdl.add_extern_service(name, portname, binding,
                location)
        self.sd.append(ServiceDefinition(self.wsdl, service))
    ...
    def send(self, soapenv):
        ...
        try:
            ...
            if e.httpcode in (202,204):
                ...
                fault = self.failed(binding, e)
                if fault[0]:
                    result = (fault[1], fault[2])
                else:
                    ctx = plugins.message.received(reply=fault[2])
                    reply = ctx.reply
                    if retxml:
                        result = reply
                    else:
