...
class ServiceDefinition(UnicodeMixin):
    ...
    def method_pprint(self, method, methodindent_n, max_char_width=80):
        """
        Return a pretty printed method signature.
        @param method: A schema type.
        @type method: method tuple.
        @param methodindent_n: The indentation level of a method.
        @type methodtype: int
        @param max_char_width: The maximum number of chars on a line.
        @type max_char_width: int
        @return: A pretty printed method string.
        @rtype: str
        """
        indent = (lambda n :  '\n%*s'%(n*3,' '))

        methodtext = '%s%s' % (indent(methodindent_n), method[0])

        # The initial width is the method name + '('
        width = len(methodtext) + 1
        params = []
        line_params = []
        for p in method[1]:
            if p[1].isattr():
                param_text = "%s _%s" % (self.xlate(p[1]), p[0])
            else:
                param_text = "%s %s" % (self.xlate(p[1]), p[0])

            if not p[1].optional():
                param_text += ' (required)'

            # Check if the the param and a comma will make the line too long.
            if len(param_text) + width + 1 > max_char_width:
                # dump out the old line
                params.append(', '.join(line_params))

                # start a new line (indented param lines are indented 2 more
                # indentation levels than method indentation).
                ind = indent(methodindent_n + 2)
                width = len(ind) + len(param_text)
                line_params = [''.join([ind, param_text])]
            else:
                # line ins't full yet...
                line_params.append(param_text)
                width += len(param_text) + 2

        # Add the last line, if it exists.
        if line_params:
            params.append(', '.join(line_params))

        return '%s(%s)' % (methodtext, ','.join(params))
