        ...
        if (len(tripleList[0]) != 0):
            ...
            try:
                ...
                ArakoonClientLogger.logError(
                    "Error while receiving from socket. %s: '%s'" % (ex.__class__.__name__, ex))
            ...
            if newChunkSize == 0:
                try:
                    ...
                    ArakoonClientLogger.logError(
                        "Error while closing socket. %s: %s" % (ex.__class__.__name__, ex))
                con._connected = False
...
def _unpackNamedField(buf, offset):
    ...
    raise ArakoonException(
        "Cannot decode named field %s. Invalid type: %d" % (name, type))
...
class ArakoonProtocol:
    ...
    def encodeRange(bKey, bInc, eKey, eInc, maxCnt, consistency):
        ...
        retVal += _packStringOption(eKey) + _packBool(
            eInc) + _packSignedInt(maxCnt)
        return retVal
    ...
    def decodeNurseryCfgResult(con):
        ...
        routing, offset = RoutingInfo.unpack(
            encoded, offset, _unpackBool, _unpackString)
