...
class ArakoonManagement:
    ...
    def start(self):
        ...
        Starts all clusters.
        """
        [clus.start() for clus in [self.getCluster(cluster)
    ...
    def stop(self):
        ...
        Stops all clusters.
        """
        [clus.stop() for clus in [self.getCluster(cluster)
                                  for cluster in self.listClusters()]]
...
class ArakoonCluster:
    ...
    def _getConfigFileName(self):
        ...
        if not p.has_section(self._clusterName):
            raise Exception("%s not present in %s" %
                            (self._clusterName, self._clustersFNH))
    ...
    def addNode(self,
        ...
        if batchedTransactionConfig is not None:
            config[name].update(
                {"batched_transaction_config": batchedTransactionConfig})
        ...
        if logDir is None:
            logDir = os.path.join(
                os.sep, 'var', 'log', 'arakoon', self._clusterName, name)
        ...
        if isWitness:
            config[name].update({"witness": "true"})

        if collapseSlowdown:
            config[name].update({"collapse_slowdown": collapseSlowdown})
        ...
        if duration:
            if not isinstance(duration, int):
                raise AttributeError(
                    "Invalid value for lease duration (expected int, got: '%s')" % duration)
    ...
    def forceMaster(self, name=None, preferred=False):
        ...
        if name:
            ...
            if not name in nodes:
                raise Exception(
                    "No node with name %s configured in cluster %s" % (name, self._clusterName))
            config.set(g, m, name)
            if preferred:
    ...
    def _changeTlogCompression(self, nodes, value):
        ...
        config = self._getConfigFile()
        for n in nodes:
            if config[n].has_key('disable_tlog_compression'):
    ...
    def disableTlogCompression(self, nodes=None):
        ...
        @param nodes List of node names
        """
        self._changeTlogCompression(nodes, 'none')
    ...
    def getClient(self):
        ...
        client = Arakoon.ArakoonClient(
            Arakoon.ArakoonClientConfig(self._clusterName, config))
        return client
    ...
    def __validateInt(self, name, value):
        ...
        if not typ == type(1):
            raise Exception(
                "%s=%s (type = %s) but should be an int" % (name, value, typ))

    def __validateName(self, name):
        if name is None or name.strip() == "":
            raise Exception(
                "A name should be passed.  An empty name is not an option")
    ...
    def _stopOne(self, name):
        ...
        while self._getStatusOne(name):
            ...
            if i == 10:
                ...
                logging.debug(msg)
                subprocess.call(
                    ['pkill', '-%d' % signal.SIGUSR2, '-f', line], close_fds=True)
                time.sleep(1)

                logging.debug("stopping '%s' with kill -9" % name)
                rc = subprocess.call(
                    ['pkill', '-9', '-f', line], close_fds=True)
                if rc == 0:
                    rc = 9
                cnt = 0
                while self._getStatusOne(name):
    ...
    def gatherEvidence(self,
        ...
        if q.qshellconfig.interactive or test:
            ...
            if not clusterCredentials:
                clusterCredentials = self._getClusterCredentials(
                    nodes_list, diff_list, test)

            elif len(clusterCredentials) < len(nodes_list):
                nodes_list = [
                    x for x in nodes_list if x not in clusterCredentials]
                diff_list = [
                    x for x in nodes_list if x not in clusterCredentials]
                sub_clusterCredentials = self._getClusterCredentials(
                    nodes_list, diff_list, test)
        ...
        for nodename in nodes_list:
            ...
            if not test:
                if nodename in diff_list:
                    ...
                    node_login = q.gui.dialog.askString(
                        "Please provide login name for %s @ %s default 'root'" % (nodename, node_ip))
                    if node_login == '':
                        node_login = 'root'

                    while node_passwd == '':
                        node_passwd = q.gui.dialog.askPassword(
                            'Please provide password for %s @ %s' % (nodename, node_ip))
                        if node_passwd == '':
                    ...
                    if len(diff_list) > 1:
                        same_credentials = q.gui.dialog.askYesNo(
                            'Do you want to set the same credentials for any other node?')

                        diff_list.remove(nodename)

                        if same_credentials:

                            same_credentials_nodes = q.gui.dialog.askChoiceMultiple(
                                "Please choose node(s) that will take same credentials:", diff_list)

                            for node in same_credentials_nodes:
                                clusterCredentials[node] = (
                                    node_login, node_passwd)
                            # end for
                            if len(same_credentials_nodes) == len(diff_list):
                                break
                            else:
                                diff_list = list(
                                    set(diff_list).difference(set(same_credentials_nodes)))
            if test:
    ...
    def _transferFiles(self,
        ...
        for nodename in nodes_list:
            ...
            source_path = 'sftp://' + userName + \
                ':' + password + '@' + source_ip

            if includeDB:
                db_files = cfs.listDir(source_path + configDict['home'])
                files2copy = filter(
                    lambda fn: fn.startswith(nodename), db_files)
            ...
            if includeLogs:
                ...
                for fname in cfs.listDir(source_path + configDict['log_dir']):
                    if fname.startswith(nodename):
                        fileinlog = q.system.fs.joinPaths(
                            configDict['log_dir'], fname)
                        cfs.copyFile(
                            source_path + fileinlog, 'file://' + node_folder)
            ...
            if includeTLogs:
                ...
                for fname in q.cloud.system.fs.listDir(full_source_dir):
                    if fname.endswith('.tlog') or fname.endswith('.tlc') or fname.endswith('.tlf'):
                        ...
                        cfs.copyFile(
                            source_path + tlogfile, 'file://' + node_folder)
            ...
            q.cloud.system.fs.copyFile(
                source_path + clusterPath, 'file://' + node_folder)

            clusterNodesPath = q.system.fs.joinPaths(
                self._clusterPath, clusterNodes)
            if q.cloud.system.fs.sourcePathExists('file://' + clusterNodesPath):
                q.cloud.system.fs.copyFile(
                    source_path + clusterNodesPath, 'file://' + node_folder)

        archive_file = sfs.joinPaths(
            q.dirs.tmpDir, self._clusterName + '_cluster_evidence.tgz')
    ...
    def setNurseryKeeper(self, clusterId):
        ...
        if len(nurseryNodes) == 0:
            raise RuntimeError(
                "A valid client configuration is required for cluster '%s'" % (clusterId))
...
if __name__ == '__main__':
    ...
    parser.add_option(
        '--stop', dest='start_stop', action="store_false", default=None,
        help="Stop arakoon")
    parser.add_option(
        '--start', dest='start_stop', action="store_true", default=None,
        help="Start arakoon")
