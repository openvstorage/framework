...
class MockStorageRouterClient(object):
    ...
    def create_snapshot(self, volume_id, snapshot_id, metadata):
        ...
        """
        snapshots = MockStorageRouterClient.snapshots[self.vpool_guid].get(volume_id, {})
        snapshots[snapshot_id] = Snapshot(metadata)
        MockStorageRouterClient.snapshots[self.vpool_guid][volume_id] = snapshots
    ...
    def create_volume(self, target_path, metadata_backend_config, volume_size, node_id):
        ...
        MockStorageRouterClient.vrouter_id[self.vpool_guid][volume_id] = node_id
        MockStorageRouterClient.volumes[self.vpool_guid][volume_id] = {'volume_id': volume_id,
    ...
    def delete_snapshot(self, volume_id, snapshot_id):
        ...
        """
        del MockStorageRouterClient.snapshots[self.vpool_guid][volume_id][snapshot_id]
    ...
    def get_dtl_config(self, volume_id):
        ...
        """
        return MockStorageRouterClient.dtl_config_cache[self.vpool_guid].get(volume_id)
    ...
    def info_snapshot(self, volume_id, snapshot_id):
        ...
        """
        return MockStorageRouterClient.snapshots[self.vpool_guid][volume_id][snapshot_id]
    ...
    def info_volume(self, volume_id):
        ...
        return type('Info', (), {'cluster_multiplier': property(lambda s: 8),
                                 ...
                                 'metadata_backend_config': property(lambda s: MockStorageRouterClient.metadata_backend_config[self.vpool_guid].get(volume_id)),
                                 'object_type': property(lambda s: MockStorageRouterClient.object_type[self.vpool_guid].get(volume_id, 'BASE')),
                                 'vrouter_id': property(lambda s: MockStorageRouterClient.vrouter_id[self.vpool_guid].get(volume_id))})()
    ...
    def is_volume_synced_up_to_snapshot(self, volume_id, snapshot_id):
        ...
        snapshot = MockStorageRouterClient.snapshots[self.vpool_guid].get(volume_id, {}).get(snapshot_id)
    ...
    def list_snapshots(self, volume_id):
        ...
        Return fake info
        """
        return MockStorageRouterClient.snapshots[self.vpool_guid].get(volume_id, {}).keys()

    def list_volumes(self):
        """
        Return a list of volumes
        """
        return [volume_id for volume_id in MockStorageRouterClient.volumes[self.vpool_guid].iterkeys()]
    ...
    def set_volume_as_template(self, volume_id):
        ...
        timestamp = 0
        newest_snap_id = None
        for snap_id, snap_info in MockStorageRouterClient.snapshots[self.vpool_guid].get(volume_id, {}).iteritems():
        ...
        if newest_snap_id is not None:
            for snap_id in copy.deepcopy(MockStorageRouterClient.snapshots[self.vpool_guid].get(volume_id, {})).iterkeys():
                if snap_id != newest_snap_id:
                    MockStorageRouterClient.snapshots[self.vpool_guid][volume_id].pop(snap_id)
        MockStorageRouterClient.object_type[self.vpool_guid][volume_id] = 'TEMPLATE'

    def unlink(self, devicename):
        """
        Delete a volume
        """
        for volume_id, volume_info in MockStorageRouterClient.volumes[self.vpool_guid].iteritems():
            if volume_info['target_path'] == devicename:
                del MockStorageRouterClient.volumes[self.vpool_guid][volume_id]
                break
    ...
    def update_metadata_backend_config(self, volume_id, metadata_backend_config):
        ...
        """
        MockStorageRouterClient.metadata_backend_config[self.vpool_guid][volume_id] = metadata_backend_config
...
class MockMetadataServerClient(object):
    ...
    def clean():
        ...
        Clean everything up from previous runs
        """
        MockMetadataServerClient.catchup = {}
    ...
    def catch_up(self, volume_id, dry_run):
        ...
        """
        _ = self, dry_run
        return MockMetadataServerClient.catchup[volume_id]
    ...
    def create_namespace(self, volume_id):
        ...
        """
        _ = self
        MockMetadataServerClient.catchup[volume_id] = 0
