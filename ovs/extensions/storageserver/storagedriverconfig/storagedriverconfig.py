...
from ovs.extensions.storageserver.storagedriverconfig.backendconnection_config import BackendConnectionConfig
from ovs.extensions.storageserver.storagedriverconfig.backend_interface_config import BackendInterfaceConfig
...
class DtlConfig(GenericConfig):
    ...
    def __init__(self, dtl_path, dtl_transport=None, **kwargs):
        """
...
class DlsConfig(GenericConfig):
    def __init__(self, dls_type=None, dls_arakoon_timeout_ms=None, dls_arakoon_cluster_id=None, dls_arakoon_cluster_nodes=None, **kwargs):
        """
...
class S3ConnectionConfig(object):
    def __init__(self, s3_connection_host=None, s3_connection_port=None, s3_connection_use_ssl=None, s3_connection_flavour=None, s3_connection_username=None,
                 s3_connection_password=None, s3_connection_ssl_cert_file=None, s3_connection_ssl_verify_host=None, s3_connection_verbose_logging=None, **kwargs):
        """
...
class NetworkConfig(GenericConfig):
    def __init__(self, network_uri=None, network_xio_slab_config=None, network_workqueue_max_threads=None, network_snd_rcv_queue_depth=None,
                 network_max_neighbour_distance=None, network_workqueue_ctrl_max_threads=None, **kwargs):
        """
...
class MdsConfig(GenericConfig):
    def __init__(self, mds_nodes=None, mds_db_type=None, mds_threads=None, mds_poll_secs=None, mds_cached_pages=None, mds_timeout_secs=None, **kwargs):
        """
...
class EventsConfig(GenericConfig):
    def __init__(self, events_amqp_uris=None, events_amqp_exchange=None, events_amqp_routing_key=None, **kwargs):
        """
...
class ScoCacheConfig(GenericConfig):
    def __init__(self, backoff_gap, trigger_gap, scocache_mount_points, **kwargs):
        """
        Initiate the volumedriverfs config: events config manager
...
class FiledriverConfig(GenericConfig):
    def __init__(self, fd_namespace, fd_cache_path, fd_extent_cache_capacity=None):
        """
...
class ScrubManagerConfig(GenericConfig):
    def __init__(self, scrub_manager_interval=None, scrub_manager_sync_wait_secs=None, scrub_manager_max_parent_scrubs=None, **kwargs):
        """
        Initiate the volumedriverfs config:  filedriver config
...
class ContentCacheConfig(GenericConfig):
    def __init__(self, read_cache_serialization_path=None, serialize_read_cache=None, clustercache_mount_points=None, **kwargs):
        """
        Initiate the volumedriverfs config:  filedriver config
...
class StorageDriverConfig(object):
    def __init__(self, local_connection_path, vrouter_cluster_id,
                 backend_config, vrouter_config, volume_manager_config, filesystem_config, dtl_config, backend_interface_config, mds_config, dls_config,
                 events_config, network_config, vregistry_config, filedriver_config, contentcache_config, scrub_manager_config,
                 *args, **kwargs):

        self.local_connection_path = local_connection_path  #enkel voor local backend
        self.vrouter_cluster_id = vrouter_cluster_id

        # Optional params

        self.backend_type = kwargs.get('backend_type', None)
        self.num_threads = kwargs.get('num_threads', None)
        self.shm_region_size = kwargs.get('shm_region_size', None)

        self.fuse_min_workers = kwargs.get('fuse_min_workers', None)
        self.fuse_max_workers = kwargs.get('fuse_max_workers', None)

        self.stats_collector_destination = kwargs.get('stats_collector_destination', None)
        self.stats_collector_interval_secs = kwargs.get('stats_collector_interval_secs', None)

        self.asio_service_manager_threads = kwargs.get('asio_service_manager_threads', None)
        self.asio_service_manager_io_service_per_thread = kwargs.get('asio_service_manager_io_service_per_thread', None)

        self.bgc_threads = kwargs.get('bgc_threads')
        ...
        self.backend_interface_config = backend_interface_config
        ...
        self.contentcache_config = contentcache_config
        self.scrub_manager_config = scrub_manager_config
        self._config = {}
        ...
        self._config = {'asio_service_manager': {'asio_service_manager_io_service_per_thread': self.asio_service_manager_io_service_per_thread,
                        ...
                        'backend_connection_manager': {'0': #todo what to do with the proxies?
                                                           self.backend_config.get_config(),
                                                       'backend_type': u'MULTI'}.update(self.backend_interface_config.get_config()),
                                                        # todo i solved merging 2 dicts this way. is this ok
                                                        # todo what to do with the backend type multi? as this is not a param
                        ...
                        'filesystem': filesystem_config.get_config(),
                        ...
                        'scocache': scocache_config.get_config(),
                        'scrub_manager': scrub_manager_config,
...
# todo dont mind this, !draft!
if __name__ == '__main__':
    my_dict = {'dtl_path' : 'dtl_path',
        'tlog_path' : 'tlog_path',
        'vrouter_id' : 'vrouter_id',
        'trigger_gap' :'trigger_gap',
        'backoff_gap' :'backoff_gap',
        'fd_namespace' :'fd_namespace',
        'fd_cache_path' : 'fd_cache_path',
        'metadata_path' : 'metadata_path',
        'clean_interval' : 'clean_interval',
        'vrouter_cluster_id' : 'vrouter_cluster_id',
        'scocache_mount_points' : 'scocache_mount_point',
        'vregistry_arakoon_cluster_id' : 'vregistry_arakoon_cluster_id',
        'read_cache_serialization_path' : 'read_cache_serialization_path',
        'local_connection_path' : 'local_connection_path',
        'vregistry_arakoon_cluster_nodes' : 'vregistry_arakoon_cluster_nodes'}
    import pprint
    backend_config = BackendConnectionConfig(**my_dict)
    ...
    backend_interface_config = BackendInterfaceConfig(**my_dict)
    ...
    pprint.pprint(StorageDriverConfig(backend_config=backend_config,
                                      ...
                                      backend_interface_config=backend_interface_config,
