...
class RelationMapper(StoredObject):
    ...
    def load_foreign_relations(object_type):
        key = 'ovs_relations_%s' % object_type.__name__.lower()
        relation_info = StoredObject.volatile.get(key)
        if relation_info is None:
            relation_info = {}
            for cls in HybridRunner.get_hybrids():
                for key, item in cls._blueprint.iteritems():
                    if isinstance(item, Relation) and item.object_type == object_type:
                        relation_info[item.remote_key] = {'class': Descriptor(cls).descriptor,
                                                          'key'  : key}
            StoredObject.volatile.set(key, relation_info)
