...
define([
    ...
    return function(options) {
        ...
        data.dedupeModes(['dedupe', 'non_dedupe']);
        data.distributedMtpt(undefined);
        data.dtlEnabled(false);
        data.dtlLocation('');
        data.dtlMode('no_sync');
        data.dtlModes(['no_sync', 'a_sync', 'sync']);
        data.extendVpool(false);
        ...
        data.port(80);
        data.readCacheSize(1);
        data.readCacheAvailableSize(undefined);
        data.scoSize(4);
        data.scoSizes([4, 8, 16, 32, 64, 128]);
        data.secretkey('');
        ...
        data.writeCacheSize(1);
        data.writeCacheAvailableSize(undefined);
    };
});
